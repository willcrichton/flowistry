...
import { cargo_command, exec_notify, get_flowistry_opts } from "../../../setup";
 ...
 */
export const get_flowistry_output = async ({
  test,
  file,
  flowistry_subcmd,
  selection,
}: TestCommand): Promise<string> => {
  ...
  const output = await exec_notify(
    cargo,
    [...cargo_args, "flowistry", flowistry_subcmd, file, start.toString()],
    test,
    command_opts
  );
 ...
 */
export const get_command_selections = async (
  test_command: TestCommand
): Promise<TestCommandResult> => {
  await perform_vscode_command(
    test_command.vscode_cmd,
    test_command.selection,
    test_command.file
  );
  ...
  const vscode_ranges = sorted_ranges.map((range) =>
    to_vsc_range(range, vscode.window.activeTextEditor?.document!)
  );
  const merged_ranges = merge_ranges(vscode_ranges);
  const expected_selections = merged_ranges.map(
 ...
 */
export const resolve_sequentially = async <T, R>(
  items: T[],
  resolver: (arg0: T) => PromiseLike<R>
): Promise<R[]> => {
  const results: R[] = [];
...
export const expect_commands = (commands: TestCommand[]) => async () => {
  const selections = await resolve_sequentially(
    commands,
    get_command_selections
  );

  selections.forEach(async (selection) => {
    expect(selection.expected_selections).to.be.deep.equalInAnyOrder(
      selection.actual_selections
    );
  });
};
