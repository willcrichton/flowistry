...
fn run(src: impl AsRef<str>, mut range: Range, lines: Vec<usize>) {
  ...
  let inner = move || -> Result<()> {
    ...
    let src = format!("fn main() {{ {} }}", src.as_ref().trim());
    ...
    let path = f.path();
    let args = format!(
      "--edition=2018 --crate-name tmp {} --sysroot {}",
      path.display(),
      sysroot
    );
...
"#;
  ...
  run(src, Range::line(4, 9, 10), vec![1, 2, 3, 4]);
}
