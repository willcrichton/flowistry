...
/// Hack to ensure that Cargo doesn't cache our plugin outputs and refuse
/// to rerun on a given file.
const NOCACHE_FLAG: &str = "--nocache";
...
pub fn cli_main<T: RustcPlugin>(plugin: T) {
  ...
  cmd.args(&["-q", "--target-dir"]).arg(target_dir);
  ...
  if single_file {
    // If we're executing on a single file, make sure to pass in the nocache
    // flag so we can re-execute without getting cached.
    cmd.arg(format!(
      "{NOCACHE_FLAG}={}",
...
pub fn driver_main<T: RustcPlugin>(plugin: T) {
  ...
  exit(rustc_driver::catch_with_exit_code(move || {
    ...
    // If we're executing on a single file, then we need to remove the
    // nocache flag.
    orig_args.retain(|flag| !flag.starts_with(NOCACHE_FLAG));
