    ...
    tls::with_opt(|opt_tcx| match opt_tcx {
      Some(tcx) => match self {
        DepNode::Place { place, at } => {
          let place_str = {
            let body = borrowck_facts::get_body_with_borrowck_facts(tcx, at.function);
        ...
        DepNode::Op(global_loc) => {
          let loc_str = match global_loc.location {
            ...
            LocationOrStart::Location(loc) => {
              let body =
                borrowck_facts::get_body_with_borrowck_facts(tcx, global_loc.function);
          ...
          write!(
            f,
            "{loc_str} @ {}",
            tcx.item_name(global_loc.function.to_def_id())
          )
        }
      },
      None => todo!(),
