#![allow(dead_code)]
use rustc_mir::borrow_check::ArtificialField;
// use rustc_mir::borrow_check::Overlap;
use rustc_hir as hir;
use rustc_middle::mir::{Body, BorrowKind, Local, Place, PlaceElem, PlaceRef, ProjectionElem};
use rustc_middle::ty::{self, TyCtxt};
use rustc_mir::borrow_check::AccessDepth::{self, Deep, Shallow};
use std::cmp::max;
...
fn place_components_conflict<'tcx>(
  ...
  match place_base_conflict(borrow_local, access_local) {
    Overlap::Arbitrary => {
      panic!("Two base can't return Arbitrary");
    }
  ...
  for (i, (borrow_c, &access_c)) in borrow_place
    .projection
    .iter()
    .zip(access_place.projection.iter())
    .enumerate()
  {
      ...
      .enumerate()
    {
      ...
      match (elem, &base_ty.kind(), access) {
        ...
        (ProjectionElem::Deref, ty::Ref(_, _, hir::Mutability::Not), _) => {
          ...
          // panic!("Tracking borrow behind shared reference.");
          return false;
        }
